...
# A utility function to printall nodes on a given line_no
...
def printVerticalLine(node,line_no,hd):
    ...
    # If this node is on given line number
    if hd == line_no:
        print(node.data,end=" ")
...
def verticalOrder(root):
    # Find min and max distance with respect to root
    minimum = [0]
    maximum = [0]
    findMinMax(root,minimum,maximum,0)
    
    # Iterate through all possible lines starting
    # from leftmost line and print nodes line by line
